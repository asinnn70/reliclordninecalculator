...
export default async function handler(req, res) {
  ...
  if (isFetching) {
    // Return stale data while fetching to prevent multiple concurrent requests
    return res.status(200).json({ type: "UPDATE", data: marketPrices });
  }
  ...
  try {
    for (const item of ITEMS) {
      ...
      if (response.ok) {
        ...
        const itemsList = data.content?.filter(x => x.item?.name === item.name) || [];
        
        if (itemsList.length > 0) {
          const cheapest = itemsList.reduce((min, current) => {
            return current.cryptoPriceInfo.price < min.cryptoPriceInfo.price ? current : min;
          });
          marketPrices[item.tier].price = cheapest.cryptoPriceInfo.price;
        }
      }
      marketPrices[item.tier].lastUpdated = Date.now();
